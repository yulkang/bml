...
%                 warning('Name unset for a VisitableTree node! Defaulting to ''root''...');
        ...
        function add_children_props(Tree, props)
            ...
            for prop = props'
                assert(isa(Tree.(prop{1}), 'bml.oop.VisitableTree'));
        ...
        function add_children(Tree, children)
            ...
            if isa(children, 'bml.oop.VisitableTree')
                children = {children};
            end
            if iscell(children)
                for child = children(:)'
        ...
        function add_child(Tree, child, name, auto_add_name)
            ...
            assert(isa(child, 'bml.oop.VisitableTree') && isscalar(child));
        ...
        function child = parse_child(Tree, child_or_name)
            if isa(child_or_name, 'bml.oop.VisitableTree')
                child = child_or_name;
    ...
    methods (Sealed)
        ...
        function set_root_(Tree, root)
            assert(isempty(root) || isa(root, 'bml.oop.VisitableTree'));
            Tree.root_ = root;
        end
